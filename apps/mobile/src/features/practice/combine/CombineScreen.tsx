...
function CombinePlay({ session, seed }: { session: GameSession; seed?: number }) {
  ...
  return (
    ...
    >
      <View testID={testIds.combine.root} style={styles.box}>
        <TalkiText align="center">בוחרים מילה קטנה, ואז תמונה — ומקבלים משפט</TalkiText>
        <View style={styles.mods}>
          {MODIFIERS.map((m, index) => (
            <Pressable
              ...
              style={[styles.mod, state.mod === m.w && styles.modSel]}
            >
              <WordArt word={{ word: m.w, emoji: m.emoji, img: m.img }} size={36} />
              <TalkiText>{display(m.w, niqqud)}</TalkiText>
            </Pressable>
          ))}
        </View>
        <TalkiText testID={testIds.combine.phrase} align="center" style={styles.phrase}>
          {state.phrase}
        </TalkiText>
        <View style={styles.grid}>
          {state.pics.map((p, index) => (
            ...
            >
              ...
              <TalkiText>{display(p.word, niqqud)}</TalkiText>
            </Pressable>
          ))}
        </View>
      </View>
...
const styles = StyleSheet.create({
  box: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 10, padding: 12 },
  mods: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 8 },
  mod: {
    minHeight: 48,
    paddingInline: 10,
    paddingBlock: 8,
  ...
  phrase: { minHeight: 28, fontSize: 20 },
